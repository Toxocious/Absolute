<?php
  ...
  class Amnesia extends Battle
  {
    ...
    )
    {
      ...
      if ( $Attacker->Stats['Sp_Defense']->Stage >= 6 )
      {
        $Effect_Text = "{$Attacker->Display_Name}'s Special Defense can't go any higher!";
      }
      else
      {
        $Stages = $this->Sp_Defense_Boost;
        if ( $Attacker->Ability == 'Simple' )
          $Stages *= 2;

        $Attacker->Stats['Sp_Defense']->SetValue($Stages);

        $Effect_Text = "{$Attacker->Display_Name}'s Special Defense rose sharply!";
      }

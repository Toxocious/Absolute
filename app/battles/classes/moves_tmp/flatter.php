<?php
  ...
  class Flatter extends Battle
  {
    ...
    )
    {
      ...
      {
        $Effect_Text .= "{$Defender->Display_Name}'s Special Attack can't go any higher!";
      }
      else
      {
        $Stages = $this->Sp_Attack_Boost;
        if ( $Defender->Ability == 'Simple' )
          $Stages *= 2;

        $Defender->Stats['Sp_Attack']->SetValue($Stages);

        $Effect_Text .= "{$Defender->Display_Name}'s Special Attack rose sharply!";
      }

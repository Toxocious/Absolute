  ...
  class Attract extends Battle
  {
    ...
    )
    {
      ...
      (
        ...
        in_array($Defender->Ability, ['Aroma Veil', 'Oblivious']) ||
        $Defender->HasStatus($this->Ailment)
      )
      {
      ...
      {
        $Set_Defender_Status = $Defender->SetStatus($this->Ailment);
        if ( $Set_Defender_Status )
          $Effect_Text .= "{$Defender->Display_Name} is now confused!";

        if ( $Defender->Item->Name == 'Destiny Knot' )
        {
          $Set_Attacker_Status = $Attacker->SetStatus($this->Ailment);
          if ( $Set_Attacker_Status )
            $Effect_Text .= "{$Attacker->Display_Name} became confused due to {$Defender->Display_Name}'s Destiny Knot!";
        }
      }

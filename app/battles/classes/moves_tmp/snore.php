  ...
  class Snore extends Battle
  {
    ...
    )
    {
      ...
      (
        ...
        !$Attacker->HasStatus('Throat Chop') &&
      ...
      )
      {
        ...
        {
          if ( $Attacker->Stats['Sp_Attack']->Stage <= 6 )
          {
            $Stages = $this->Sp_Attack_Boost;
            if ( $Attacker->Ability == 'Simple' )
              $Stages *= 2;

            $Attacker->Item->Consume();

            $Attacker->Stats['Sp_Attack']->SetValue($Stages);

            $Effect_Text = "{$Attacker->Display_Name}'s Special Attack rose sharply!";
          }
        }

        if ( $Attacker->Ability == 'Punk Rock' )
          $this->Power *= 1.3;

        $Damage = $this->CalcDamage($Side, $STAB, $Does_Move_Crit, $Move_Effectiveness['Mult']);

        if ( $Defender->Ability == 'Punk Rock' )
          $Damage /= 2;
      }
      else
      {

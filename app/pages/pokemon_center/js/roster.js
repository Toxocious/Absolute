    ...
    await SendRequest('roster', Form_Data)
        .then((Roster_Data) => {
            ...
            for (let Slot = 0; Slot < 6; Slot++) {
                ...
                document
                    .getElementById(`Roster_Slot_${Slot + 1}_Icon`)
                    .setAttribute('src', `${Icon_Path}/${Icon_Name}.png`);
                document.getElementById(`Roster_Slot_${Slot + 1}_Display_Name`).innerText =
                    Display_Name;

                if (typeof Pokemon_Slot !== 'undefined') {
                    document.getElementById(
                        `Roster_Slot_${Slot + 1}_Level`
                    ).innerText = `Level: ${Pokemon_Slot.Level}`;

                    const Move_Options = [1, 2, 3, 4, 5, 6, 7];
                    for (const Move_Option of Move_Options) {
                        document
                            .getElementById(`Roster_Slot_${Slot + 1}_Move_To_${Move_Option}`)
                            .setAttribute(
                                'onclick',
                                `MovePokemon(${Pokemon_Slot.ID}, ${Move_Option})`
                            );
                    }
                }
            }
...
async function PreviewPokemon(Pokemon_ID) {
    ...
    await SendRequest('roster', Form_Data)
        .then((Preview_Pokemon) => {
            ...
            document.querySelector('#Pokemon_Preview > tr > td').innerHTML =
                Preview_Pokemon.Pokemon_Data;
        })
        .catch((Error) => console.error('Error:', Error));
}

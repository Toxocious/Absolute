...
async function GetReleasablePokemon() {
    ...
    await SendRequest('pokemon_center', 'release', Form_Data)
        .then((Releasable_Pokemon) => {
            Releasable_Pokemon = JSON.parse(Releasable_Pokemon)[0];

            document.getElementById(`Total_Releasable_Pokemon`).innerText =
                Releasable_Pokemon.Amount ?? '0';

            if (Releasable_Pokemon.Pokemon.length > 0) {
                document.getElementById('Release_Button').disabled = false;
                document
                    .getElementById('Release_Button')
                    .setAttribute('onclick', 'SelectPokemonForRelease()');
...
async function SelectPokemonForRelease() {
    ...
    if (Selected_Pokemon_Amount < 1) return;
    ...
    await SendRequest('pokemon_center', 'release', Form_Data)
        .then((Processed_Pokemon) => {
            ...
            if (Processed_Pokemon.Success != null) {
                document.getElementById('Pokemon_Center_Moves_AJAX').className =
                    Processed_Pokemon.Success ? 'success' : 'error';
                document.getElementById('Pokemon_Center_Moves_AJAX').innerHTML =
                    Processed_Pokemon.Message;
            }
...
async function FinalizeRelease() {
    ...
    await SendRequest('pokemon_center', 'release', Form_Data)
        .then((Release_Pokemon) => {
            ...
            document.getElementById('Pokemon_Center_Release_AJAX').className =
                Release_Pokemon.Success ? 'success' : 'error';
            document.getElementById('Pokemon_Center_Release_AJAX').innerHTML =
                Release_Pokemon.Message;

        ...
        .then((Roster_Data) => {
            ...
            for (let Slot = 0; Slot < 6; Slot++) {
                ...
                if (typeof Pokemon_Slot === 'undefined') {
                    ...
                    if (Pokemon_Slot.Forme) Sprite_Name += `-${Pokemon_Slot.Forme}`;
                ...
                document
                    .getElementById(`Roster_Slot_${Slot + 1}_Sprite`)
                    .setAttribute('src', `${Sprite_Path}/${Sprite_Name}.png`);
                document.getElementById(`Roster_Slot_${Slot + 1}_Display_Name`).innerText =
                    Display_Name;

                if (typeof Pokemon_Slot !== 'undefined') {
                    Object.keys(Pokemon_Slot.Move_Data).forEach(function (Move_Slot) {
                        document
                            .getElementById(`Roster_Slot_${Slot + 1}_Move_${Move_Slot}`)
                            .setAttribute(
                                'onclick',
                                `SelectMoveSlot(${Slot + 1}, ${Pokemon_Slot.ID}, ${Move_Slot})`
                            );
                        document.getElementById(
                            `Roster_Slot_${Slot + 1}_Move_${Move_Slot}`
                        ).innerHTML = `<b>${Pokemon_Slot.Move_Data[Move_Slot].Name}</b>`;
                    });
                }
            }
        })
...
async function UpdateMoveSlot(Pokemon_ID, Move_Slot) {
    ...
    await SendRequest('pokemon_center', 'moves', Form_Data)
        .then((Move_Data) => {
            ...
            document.getElementById('Pokemon_Center_Moves_AJAX').className = Move_Data.Success
                ? 'success'
                : 'error';

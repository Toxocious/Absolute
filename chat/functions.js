...
module.exports = {
log: function(text) {
	fs.appendFile('log.txt', text+os.EOL, encoding='utf8', function (err) {
		if (err) 
			throw err;
	});
	console.log(text);
},
logSync: function(text) {
	fs.appendFileSync('log.txt', text+os.EOL, encoding='utf8', function (err) {
		if (err) 
			throw err;
	});
	console.log(text);
...
getPokeIcon: function(PokeID, AltID, Type) {
	...
	return 'http://sprites.tpkrpg.net/pokemon/icons/'+Type.toLowerCase()+'/'+Image+'.png';
},
/* **********************

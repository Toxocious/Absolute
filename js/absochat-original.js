...
Chaterpie = {
  ...
  enable: function() {
    ...
    if (typeof socket != "object") {
      ...
      socket = io('localhost:' + CHATERPIE_PORT, {
        // reconnection: true,
        secure: true
      });
...
ChatMessage = {
  ...
  display: function() {
    ...
    $('#chatContent').find('.Status').first().html('<div class="chatWindow" style="'+s+';">' + (ChatMessage.temp_chat) + "</div>");
...
function sendEmote(e) {
  $('#chatMessage').val($('#chatMessage').val() + e).keyup()
  return false;
}

  ...
  class Sketch extends Battle
  {
    ...
    )
    {
      ...
      else if ( isset($Defender->Last_Move) )
      {
        ...
        {
          if ( $Foes_Last_Move->Name == 'Chatter' )
          {
            $Effect_Text = 'But it failed!';
          }
          else
          {
            $Attacker->Moves[$this->Slot] = $Foes_Last_Move;

            $Effect_Text = "{$Attacker->Display_Name} sketched {$Defender->Display_Name}'s {$Foes_Last_Move->Name}!";
          }
        }
        else
        {

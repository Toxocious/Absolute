  ...
  class Present extends Battle
  {
    ...
    )
    {
      ...
      if ( $Probability <= 2 )
      {
        ...
        {
          $Effect_Text = "{$Defender->Display_Name} couldn't receive the gift!";
        }
        else
        {
          $Defender->IncreaseHP($Defender->Max_HP / 4);

          $Effect_Text = "{$Defender->Display_Name} had its HP restored!";
        }
      }
      else
      {
      ...
      return [
        'Text' => "{$Attacker->Display_Name} used {$this->Name} and dealt <b>" . number_format($Damage) . "</b> damage to {$Defender->Display_Name}." .
                  (isset($Damage) && $Move_Effectiveness['Text'] != '' ? "<br />{$Move_Effectiveness['Text']}" : '') .
                  (isset($Damage) && $Does_Move_Crit ? '<br />It critically hit!' : ''),

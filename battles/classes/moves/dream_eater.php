<?php
  ...
  class Dream_Eater extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      if
      (
        $Defender->HasStatus('Heal Block') ||
        (!$Defender->HasStatus('Sleep') && $Defender->Ability->Name != 'Comatose')
      )
      {
      ...
      {
        $Effect_Text = "{$Attacker->Display_Name} ate {$Defender->Display_Name}'s dreams!";

        $Damage = $this->CalcDamage($Side, $STAB, $Does_Move_Crit, $Move_Effectiveness['Mult']);

        if ( $Attacker->Item->Name == 'Big Root' )
          $Drain_Mult = round(30 / 100, 2) * ($Damage / 2);
        else
          $Drain_Mult = 1;

        if ( $Defender->Ability->Name == 'Liquid Ooze' )
        {
          $Attacker->DecreaseHP($Damage * $Drain_Mult);
          $Effect_Text .= "<br />{$Attacker->Display_Name} took damage from the Liquid Ooze!";
        }
        else
        {
          $Attacker->IncreaseHP($Damage * $Drain_Mult);
          $Effect_Text .= "<br />{$Attacker->Display_Name} regained health!";
        }
      }

<?php
  ...
  class Rest extends Battle
  {
    ...
    )
    {
      ...
      if ( $this->Weather == 'Harsh Sunlight' )
        if ( $Attacker->Active->Ability == 'Leaf Guard' )
          $Effect_Text = 'But if failed!';
      else if ( in_array($this->Terrain, ['Electric', 'Misty']) && $Attacker->Active->HasStatus('Grounded') )
        $Effect_Text = 'But it failed!';
      else if ( in_array($Attacker->Active->Ability, ['Insomnia', 'Vital Spirit']) )
        $Effect_Text = 'But it failed!';
      else if ( $Attacker->IsFieldEffectActive('Sweet Veil') )
        $Effect_Text = 'But it failed!';
      else if ( $Attacker->Active->HasStatus('Uproar') && $Attacker->Active->Ability != 'Soundproof' )
        $Effect_Text = 'But it failed!';
      else if ( $Attacker->Active->HasStatus('Uproar') && $Attacker->Active->Ability == 'Soundproof' )
        $Effect_Text = "{$Attacker->Active->Display_Name} fell asleep!";
      else
      {
        $Attacker->Active->IncreaseHP($Attacker->Active->Max_HP);
        $Attacker->Active->RemoveStatusFromArray(['Burn', 'Paralysis', 'Poison', 'Freeze']);

        $Effect_Text = "{$Attacker->Active->Display_Name} slept and became healthy!";
      }

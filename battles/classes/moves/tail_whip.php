<?php
  ...
  class Tail_Whip extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      {
        if ( $Defender->Active->Stats['Defense']->Stage <= -6 )
        {
          $Effect_Text = "{$Defender->Active->Display_Name}'s Defense can't go any lower!";
        }
        else
        {
          $Stages = $this->Defense_Boost;

          $Defender->Active->Stats['Defense']->SetValue($Stages);

          $Effect_Text = "{$Defender->Active->Display_Name}'s Defense has fallen!";
        }
      }

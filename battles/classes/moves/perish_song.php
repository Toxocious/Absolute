<?php
  ...
  class Perish_Song extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      $Effect_Text = "All Pok&eacute;mon hearing the song will faint in three turns!";

      foreach ( [$Attacker, $Defender] as $Battle_Side )
      {
        $Perish_Song_Status = $Battle_Side->HasStatus('Perish Song');
        if ( $Perish_Song_Status )
        {
          if ( $Perish_Song_Status->Turns_Left > 0 )
            $Perish_Song_Status->UpdateStatus();
        }
        else
        {
          $Battle_Side->SetStatus('Perish Song');
        }
      }

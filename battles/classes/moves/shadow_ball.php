<?php
  ...
  class Shadow_Ball extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      if ( $Defender->Ability == 'Bulletproof' )
      {
        $Effect_Text = 'But it failed!';
      }
      else
      {
        if ( $Defender->Stats['Sp_Defense']->Stage <= -6 )
        {
          $Effect_Text = "{$Defender->Display_Name}'s Special Defense can't go any lower!";
        }
        else
        {
          $Stages = $this->Sp_Defense_Boost;

          $Defender->Stats['Sp_Defense']->SetValue($Stages);

<?php
  ...
  class Future_Sight extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      if ( $Defender->HasStatusFromArray(['Future Sight', 'Doom Desire']) )
      {
        $Effect_Text = 'But it failed!';
      }
      else
      {
        $Defender->SetStatus('Future Sight');

        $Effect_Text = "{$Attacker->Display_Name} has foreseen an attack!";
      }

<?php
  ...
  class Spite extends Battle
  {
    ...
    )
    {
      ...
      if ( $Foes_Last_Move->Current_PP > 0 )
      {
        $Foes_Last_Move->Current_PP -= 4;

        if ( $Foes_Last_Move->Current_PP < 0 )
          $Foes_Last_Move->Current_PP = 0;

        $Effect_Text = "{$Defender->Display_Name}'s {$Foes_Last_Move->Name} PP was reduced by 4!";
      }
      else
      {

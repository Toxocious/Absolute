<?php
  ...
  class Sweet_Scent extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      {
        if ( $Defender->Stats['Evasion']->Stage <= -6 )
        {
          $Effect_Text = "{$Defender->Display_Name}'s Evasiveness can't go any higher!";
        }
        else
        {
          $Stages = $this->Evasion_Boost;

          $Defender->Stats['Evasion']->SetValue($Stages);

          $Effect_Text = "{$Defender->Display_Name}'s Evasiveness rose sharply!";
        }
      }

<?php
  ...
  class Skull_Bash extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      $Effect_Text = "{$Attacker->Display_Name} tucked in its head.<br />";

      if ( $Attacker->Stats['Defense']->Stage >= 6 )
      {
        $Effect_Text .= "{$Attacker->Display_Name}'s Defense can't go any higher!<br />";
      }
      else
      {
        ...
        $Effect_Text .= "{$Attacker->Display_Name}'s Defense rose sharply!<br />";
      }

      if ( !$Attacker->HasStatus('Charging') )
      {
        if ( $Attacker->Item->Name != 'Power Herb' )
        {

  ...
  class Swagger extends Battle
  {
    ...
    )
    {
      ...
      if ( $Defender->Ability == 'Contrary' )
      {
        ...
        {
          $Defender->DecreaseHP($Defender->Max_HP / 2);
          $Stages = $this->Attack_Boost;
          $Defender->Stats['Attack']->SetValue($Stages);

          $Effect_Text .= "{$Defender->Display_Name}'s Attack was minimized!";
        }
      }
      else
      {
        if ( $Defender->Stats['Attack']->Stage >= 6 )
        {
          $Effect_Text .= "{$Defender->Display_Name}'s Attack can't go any higher!";
        }
        else
        {
          $Defender->DecreaseHP($Defender->Max_HP / 2);
          ...
          $Effect_Text .= "{$Defender->Display_Name}'s Attack was maximized!";
        }
      }
      ...
      if ( !$Defender->HasStatus('Confusion') )
      {
        ...
        $Effect_Text .= "<br />{$Defender->Display_Name} became confused!";
      }

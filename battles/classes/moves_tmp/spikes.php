<?php
  ...
  class Spikes extends Battle
  {
    ...
    )
    {
      ...
      $Spikes_Field = $Defender->IsFieldEffectActive('Spikes');
      if ( $Spikes_Field )
      {
        if ( $Spikes_Field->Stacks < 3 )
        {
          ...
          $Effect_Text = "Spikes were scattered around {$Defender->Active->Display_Name}'s team!";
        }
        else
        {
      ...
      {
        $Defender->SetFieldEffect('Spikes');

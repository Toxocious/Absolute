<?php
  ...
  class Heal_Bell extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      $Effect_Text = "A bell chimed! {$Attacker->Display_Name}'s allies were cured of any ailments!";

      if ( $Attacker->Active->Ability == 'Liquid Voice' )
      {
        $this->Move_Type = 'Water';

        $Effect_Text .= "<br />{$this->Name} became a Water-type move!";
      }

      foreach ( $Attacker->Roster as $Pokemon )
      {
        if ( $Pokemon->HasStatusFromArray(['Burn', 'Freeze', 'Paralysis', 'Poison', 'Sleep']) )
        {
          $Pokemon->RemoveStatusFromArray(['Burn', 'Freeze', 'Paralysis', 'Poison', 'Sleep']);
        }
      }

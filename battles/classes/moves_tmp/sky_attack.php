<?php
  ...
  class Sky_Attack extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      if ( !$Attacker->HasStatus('Charging') )
      {
        if ( $Attacker->Item->Name != 'Power Herb' )
        {
          ...
          return [
            'Text' => "{$Attacker->Display_Name} became cloaked in a harsh light!",
            'Damage' => 0,
            'Healing' => 0,
          ];
        }
        else
        {
          ...
          if ( $Attempt_Consume )
            $Effect_Text = "{$Attacker->Display_Name} became cloaked in a harsh light!";
        }
      }

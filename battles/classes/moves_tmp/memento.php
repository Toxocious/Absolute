<?php
  ...
  class Memento extends Battle
  {
    ...
    )
    {
      ...
      foreach (['Attack', 'Sp_Attack'] as $Index => $Stat)
      {
        if ( $Defender->Stats[$Stat]->Stage > -6 )
        {
          $Stat_Boost = $Stat . '_Boost';

          $Stages = $this->$Stat_Boost;

          $Defender->Stats[$Stat]->SetValue($Stages);

          $Stat_Name = str_replace('_', 'ecial ', $Stat);
          $Effect_Text .= "{$Defender->Display_Name}'s {$Stat_Name} fell sharply!";
      ...
      $Attacker->DecreaseHP($Attacker->HP);

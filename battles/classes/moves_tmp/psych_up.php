<?php
  ...
  class Psych_Up extends Battle
  {
    ...
    )
    {
      ...
      $Stat_Change_Count = 0;
      foreach ( $Defender->Stats as $Stat )
      {
        if ( $Stat->Stage === 0 )
          continue;

        $Stat_Change_Count++;
        $Attacker->Stats[$Stat->Name]->Stage = $Stat->Stage;
      }

      if ( $Stat_Change_Count === 0 )
        $Effect_Text = 'But it failed!';
      else
        $Effect_Text = "{$Attacker->Display_Name} copied {$Defender->Display_Name}'s stat changes!";
    ...
    )
    {
      ...
      if ( $Defender->HasStatusFromArray(['Semi-Invulnerable', 'Crafty Shield', 'Sky Drop', 'Bounce', 'Dive', 'Dig', 'Fly']) )
        return false;

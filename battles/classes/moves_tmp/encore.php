<?php
  ...
  class Encore extends Battle
  {
    ...
    )
    {
      ...
      if ( $Defender->Ability == 'Aroma Veil' )
      {
        $Effect_Text = 'But it failed!';
      }
      else if ( isset($Defender->Last_Move) )
      {
        $Foes_Last_Move = $Defender->Moves[$Defender->Last_Move['Slot']];
        if ( !in_array($Foes_Last_Move, ['Mimic', 'Metronome', 'Mirror Move']) )
        {
          $Set_Status = $Defender->SetStatus('Encore', 3);
          if ( $Set_Status )
            $Effect_Text = "{$Defender->Display_Name} is now locked in an encore!";
        }
        else
        {
          $Effect_Text = 'But it failed!';

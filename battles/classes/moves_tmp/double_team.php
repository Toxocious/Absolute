<?php
  ...
  class Double_Team extends Battle
  {
    public $Name = null;
    ...
    )
    {
      ...
      if ( $Attacker->Stats['Evasion']->Stage <= -6 )
      {
        $Effect_Text = "{$Attacker->Display_Name}'s Evasiveness can't go any higher!";
      }
      else
      {
        $Stages = $this->Evasion_Boost;

        $Attacker->Stats['Evasion']->SetValue($Stages);

        $Effect_Text = "{$Attacker->Display_Name}'s Evasiveness rose sharply!";
      }

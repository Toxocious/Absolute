<?php
  ...
  class Dig extends Battle
  {
    ...
    )
    {
      ...
      if ( !$Attacker->HasStatus('Dig') )
      {
        if ( $Attacker->Item->Name != 'Power Herb' )
        {
          $Attacker->SetStatus('Dig');

          return [
            'Text' => "{$Attacker->Display_Name} has dug underground!",
            'Damage' => 0,
            'Healing' => 0,
          ];
        }
        else
        {
          ...
          if ( $Attempt_Consume )
            $Effect_Text = "{$Attacker->Display_Name} has dug underground!";
        }
      }

      $Attacker->RemoveStatus('Dig');

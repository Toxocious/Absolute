...
(function() {
  ...
  var DebugPanel = me.Renderable.extend({
    ...
    init: function(debugToggle) {
      ...
      this.debugToggle = debugToggle || me.input.KEY.B;
      this.keyHandler = me.event.subscribe(me.event.KEYDOWN, function(
        action,
        keyCode
      ) {
      ...
      this.help_str_len = this.font.measureText(
        me.video.renderer,
        this.help_str
      ).width;
      this.fps_str_len = this.font.measureText(
        me.video.renderer,
        "00/00 fps"
      ).width;
    ...
    patchSystemFn: function() {
      ...
      me.debug.renderHitBox =
        me.debug.renderHitBox || me.game.HASH.hitbox || true;
      me.debug.renderVelocity =
        me.debug.renderVelocity || me.game.HASH.velocity || false;
      me.debug.renderQuadTree =
        me.debug.renderQuadTree || me.game.HASH.quadtree || false;
        ...
        if (_this.visible) {
          ...
          if (me.debug.renderVelocity && (this.body.vel.x || this.body.vel.y)) {
            ...
            renderer.strokeLine(
              0,
              0,
              ~~(this.body.vel.x * (bounds.width / 2)),
              ~~(this.body.vel.y * (bounds.height / 2))
            );
    ...
    draw: function(renderer) {
      ...
      this.font.draw(
        renderer,
        "?hitbox   [" + (me.debug.renderHitBox ? "x" : " ") + "]",
        75 * this.mod,
        2 * this.mod
      );
      this.font.draw(
        renderer,
        "?velocity [" + (me.debug.renderVelocity ? "x" : " ") + "]",
        75 * this.mod,
        10 * this.mod
      );
      ...
      this.font.draw(
        renderer,
        "?QuadTree [" + (me.debug.renderQuadTree ? "x" : " ") + "]",
        150 * this.mod,
        2 * this.mod
      );
  ...
  me.device.onReady(function() {
    me.utils.function.defer(
      ...
      me.debug.Panel,
      "debugPanel",
      me.game.HASH.debugToggleKey
        ? me.game.HASH.debugToggleKey.charCodeAt(0) - 32
        : undefined
    );
  });
